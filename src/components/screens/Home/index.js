...
const Home = ({ history }) => {
  ...
  const [isPredictionSheetVisible, setIsPredictionSheetVisible] = useState(
    false
  );
  ...
  useEffect(() => {
    if (authUser) {
      apiClient
        .getUserPredictions(authUser)
        .then((predictions) => setUserPredictions(predictions));
    }
  }, [authUser]);

  const toggleOverlay = () => setIsOverlayVisible(!isOverlayVisible);
  const togglePredictionSheet = () =>
    setIsPredictionSheetVisible(!isPredictionSheetVisible);
